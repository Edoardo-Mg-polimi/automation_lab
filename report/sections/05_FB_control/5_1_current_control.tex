...
In the experimental setup, the voltage applied to the coil is obtained through the power amplifier, whose gain is denoted by \(K_g\).
Therefore, the transfer function from the amplifier command to the coil current is
\begin{equation}
    K_g G_i(s) = \frac{K_g}{Ls+R}.
\end{equation}

A PI controller is selected in order to obtain zero steady-state error for constant current references:
\begin{equation}
    R_i(s) = K_p + \frac{K_i}{s}
           = K_p\left(1+\frac{1}{T_i s}\right),
\end{equation}
\textcolor{red}{WHY PI AND NOT PID?\\}
with
\begin{equation}
    K_i = \frac{K_p}{T_i}.
\end{equation}
...
In order to protect the coil, a saturation on the voltage actuation is introduced. The coil can accept $\pm V_{\max} = 24\ \mathrm{V}$ as input.
...
To reduce the windup effect, an Internal Model Control anti-windup scheme is introduced. The difference between the unsaturated and saturated voltages is filtered through an internal model of the electrical plant and added to the measured current used in the feedback path:
\begin{equation}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../img/models/BODE_Hi.png}
