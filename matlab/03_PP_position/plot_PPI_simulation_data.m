...
matFiles = dir(fullfile(scriptDir, 'PPI_NL_simulation_*.mat'));

if isempty(matFiles)
    error('No PPI_NL_simulation_*.mat files found in %s.', scriptDir);
end
...
for k = 1:numel(matFiles)
    ...
    plotMainSimulation(data, erase(matFiles(k).name, '.mat'), 'PPI');
    plotAllSignals(data, erase(matFiles(k).name, '.mat'), 'PPI');
end
